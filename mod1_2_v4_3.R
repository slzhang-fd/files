...
model{
  ...
  # Priors ---------------------------------------------------------------------
  tau.u_tp ~ dgamma(0.01, 0.01)
  tau.u_fp ~ dgamma(0.01, 0.01)
  sig2.u_tp <- 1 / tau.u_tp
  sig2.u_fp <- 1 / tau.u_fp

  rho_trans ~ dbeta(1.5, 1.5)
  rho.u <- 2 * rho_trans - 1

  Sigma.u[1,1] <- sig2.u_tp
  Sigma.u[2,2] <- sig2.u_fp
  Sigma.u[1,2] <- rho.u*sqrt(sig2.u_tp)*sqrt(sig2.u_fp)
  Sigma.u[2,1] <- Sigma.u[1,2]

  invSigma.u <- inverse(Sigma.u)
...
#monitor# alpha_tp, alpha_fp, b_tp, b_fp, beta_tp, beta_fp, g_tp, g_fp, gamma_tp, gamma_fp, Sigma.u, rho.u
"